...
export default function ListingCard({ listing, commuteTimes, commuteRank, matchScore, rankKind }: Props) {
  ...
  const badgeLabel = listing.badge
    ? ({ New: isAr?"جديد":"New", "Open Sat": isAr?"مفتوح السبت":"Open Sat", "Price Cut": isAr?"تخفيض":"Price Cut", "Hot Home": isAr?"مطلوب":"Hot Home", Exclusive: isAr?"حصري":"Exclusive", Premium: isAr?"مميز":"Premium", Featured: isAr?"مميز":"Featured" }[listing.badge] ?? listing.badge)
    : null;
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="text-lg font-black truncate" style={{ color: "var(--text)" }}>{formatListingPrice(listing.priceValue, listing.listingType, isAr)}</p>
        ...
        <div className="mt-3 flex items-center gap-3 text-sm" style={{ color: "var(--text-muted)" }}>
          ...
          <span style={{ color: "var(--border)" }}>|</span>
          <span className="capitalize">{propertyTypeLabel(listing.propertyType, isAr)}</span>
        </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-black leading-tight" style={{ color: col }}>
                      {avg} {isAr ? "دقيقة" : "min"}{valid.length > 1 ? (isAr ? " متوسط" : " avg") : ""}
                    </p>
                    <p className="text-xs leading-tight" style={{ color: "var(--text-faint)" }}>
                      {valid.length === 1
                        ? `${isAr ? "إلى" : "to"} ${valid[0].label}`
                        : `${isAr ? "متوسط · " : "avg · "}${valid.map(ct => `${ct.label} ${ct.minutes}${isAr ? "د" : "m"}`).join(isAr ? "، " : ", ")}`}
                    </p>
                  </div>
                </div>
