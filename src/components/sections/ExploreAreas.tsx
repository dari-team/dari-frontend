    ...
    <section className="py-20" style={{ backgroundColor: "var(--bg-secondary)" }}>
      <div className="container-custom">
        <div className="mb-10">
          ...
          <h2 className="text-3xl font-black" style={{ color: "var(--text)" }}>{t("sections.exploreAreas")}</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5">
          {areas.map((area) => (
            <button key={area.name} onClick={() => navigate(`/search?city=${encodeURIComponent(area.name)}`)}
              className="relative h-[260px] rounded-2xl overflow-hidden group cursor-pointer transition duration-300 hover:-translate-y-2 text-left w-full"
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end text-white">
                <div>
                  <span className="font-bold text-lg block">{area.name}</span>
                  <span className="text-xs text-white/60">{area.count} {t("search.listings")}</span>
                </div>
                <span className="opacity-0 group-hover:opacity-100 transition text-sm font-medium">View →</span>
              </div>
            </button>
          ))}
        </div>
